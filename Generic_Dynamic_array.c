...
gen_dyn_arr* create_gen_dyn_arr(size_t max_size, size_t element_size){
    ...
    gda->data = malloc(gda->max_size * element_size);
}
...
void remove_element_in_place(gen_dyn_arr* gda, size_t pos) {
    if (pos >= gda->max_size || pos < 0){
        printf("This action is not posible.(pos is out of bounds)");
        return;
    }
    if (pos < gda->max_size - 1) {  // If it's not the last element, shift everything after it one spot to the left
        char* dest = (char*)gda->data + (pos * gda->element_size);
        char* src = (char*)gda->data + ((pos + 1) * gda->element_size);
        size_t bytes_to_move = (gda->max_size - pos - 1) * gda->element_size;
...
gen_dyn_arr* resize(gen_dyn_arr* gda, size_t new_max_size){
    gen_dyn_arr* gda1 = create_gen_dyn_arr(new_max_size, gda->element_size);
    for (int i = 0; i < new_max_size; i++){
        memcpy((char*)gda1->data + i * gda->element_size, (char*)gda->data + i * gda->element_size, gda->element_size);
    }
}
...
size_t linear_search_gen_dyn_arr(gen_dyn_arr* gda, int (*cmp)(const void*, const void*), void* element, size_t element_size){
    if (element_size != gda->element_size){
        printf("Types of [element] and [gda->data] are incompatible\n");
        return -1;
    }
    for(size_t i = 0; i < gda->max_size; i++){
        if (!cmp((char*)gda->data + i * element_size, element)){
            return i;
        }
    }
    return -1;
...
size_t binary_search_gen_dyn_arr(gen_dyn_arr* gda, int (*cmp)(const void*, const void*), void* element) {
    size_t bot = 0;
    size_t top = gda->max_size - 1;

    while (bot <= top) {
        size_t mid = bot + (top - bot) / 2;
        void* mid_ptr = (char*)gda->data + (mid * gda->element_size);   // Cast to char* for byte-accurate offsetting
        int res = cmp(mid_ptr, element);    //used in the conditions
        if (res == 0) return mid;   //element found
        if (res < 0) bot = mid + 1;
        else {
            if (mid == 0) break; // Prevent underflow of size_t
            top = mid - 1;
        }
    }
    return -1; // Not found
